...
AF_BS <- read.csv('PSMC_4ind_30bs_newMutRate.csv', head = T, sep = ',')
p <- ggplot() + geom_step(data=AF_BS, mapping=aes(x=(AF_time), y=AF), color = "#654CFF", size = 1) +
  ...
  labs(x = "Time (years before present)", y = "Effective population size x 10,000", color = "Legend") + scale_color_manual(values = colors) +
  theme_classic()+
...
# Shade from 40000 to 100000 years ago 
dateRanges <- data.frame(
  from=(40000), 
  to=(100000)
)
p + scale_x_continuous(trans='log10', limits = c(1000,2000000))+ 
